...
class TreeTab_old extends ScrollableTab {
	/* Example widgets and groups that contain them */
	Tree_old tree1, tree2;
	TreeItem_old textNode1, imageNode1;
	 ...
	 */
	TreeTab_old(ControlExample instance) {
		super(instance);
	}
	...
	private void makeTreeEditable(Tree_old tree) {
	...
	private void treeDoubleClickListener(Tree_old tree, final TreeEditor editor, Event event) {
		if (!editableButton.getSelection()) {
			return;
		}
		Point point = new Point(event.x, event.y);
		TreeItem_old item = tree.getItem(point);
		if (item == null) {
			return;
		}
		// Get the item text
	...
	void createExampleGroup () {
		...
		/* Create a group for the text Tree_old */
		...
		/* Create a group for the image Tree_old */
	...
	void createExampleWidgets () {
		...
		/* Create the text Tree_old */
		tree1 = new Tree_old (treeGroup, style);
		boolean multiColumn = multipleColumns.getSelection();
		if (multiColumn) {
			for (String columnTitle : columnTitles) {
				TreeColumn_old TreeColumn_old = new TreeColumn_old(tree1, SWT.NONE);
				TreeColumn_old.setText(columnTitle);
				TreeColumn_old.setToolTipText(ControlExample.getResourceString("Tooltip", columnTitle));
			}
			tree1.setSortColumn(tree1.getColumn(0));
		}
		for (int i = 0; i < 4; i++) {
			TreeItem_old item = new TreeItem_old (tree1, SWT.NONE);
			setItemText(item, i, ControlExample.getResourceString("Node_" + (i + 1)));
			if (i < 3) {
				TreeItem_old subitem = new TreeItem_old (item, SWT.NONE);
				setItemText(subitem, i, ControlExample.getResourceString("Node_" + (i + 1) + "_1"));
			}
		}
		TreeItem_old treeRoots[] = tree1.getItems ();
		TreeItem_old item = new TreeItem_old (treeRoots[1], SWT.NONE);
		setItemText(item, 1, ControlExample.getResourceString("Node_2_2"));
		item = new TreeItem_old (item, SWT.NONE);
		setItemText(item, 1, ControlExample.getResourceString("Node_2_2_1"));
		textNode1 = treeRoots[0];
		packColumns(tree1);
		try {
			TreeColumn_old column = tree1.getColumn(0);
			resizableColumns.setSelection (column.getResizable());
		} catch (IllegalArgumentException ex) {}

		/* Create the image Tree_old */
		tree2 = new Tree_old (imageTreeGroup, style);
		Image image = instance.images[ControlExample.ciClosedFolder];
		if (multiColumn) {
			for (int i = 0; i < columnTitles.length; i++) {
				TreeColumn_old TreeColumn_old = new TreeColumn_old(tree2, SWT.NONE);
				TreeColumn_old.setText(columnTitles[i]);
				TreeColumn_old.setToolTipText(ControlExample.getResourceString("Tooltip", columnTitles[i]));
				if (headerImagesButton.getSelection()) TreeColumn_old.setImage(instance.images [i % 3]);
			}
		}
		for (int i = 0; i < 4; i++) {
			item = new TreeItem_old (tree2, SWT.NONE);
			setItemText(item, i, ControlExample.getResourceString("Node_" + (i + 1)));
		...
		item = new TreeItem_old (treeRoots[1], SWT.NONE);
		setItemText(item, 1, ControlExample.getResourceString("Node_2_2"));
		if (multiColumn && subImagesButton.getSelection()) {
			for (int j = 0; j < columnTitles.length; j++) {
				item.setImage(j, image);
	...
	void setItemText(TreeItem_old item, int i, String node) {
		int index = i % 3;
	...
	Item [] getExampleWidgetItems () {
		/* Note: We do not bother collecting the Tree_old items
		 * because Tree_old items don't have any events. If events
		 * are ever added to TreeItem_old, then this needs to change.
		 */
	...
	Object[] parameterForType(String typeName, String value, Widget widget) {
		if (typeName.equals("org.eclipse.swt.widgets.TreeItem_old")) {
			TreeItem_old item = findItem(value, ((Tree_old) widget).getItems());
			if (item != null) return new Object[] {item};
		}
		if (typeName.equals("[Lorg.eclipse.swt.widgets.TreeItem_old;")) {
			String[] values = split(value, ',');
			TreeItem_old[] items = new TreeItem_old[values.length];
			for (int i = 0; i < values.length; i++) {
				TreeItem_old item = findItem(values[i], ((Tree_old) widget).getItems());
				if (item == null) break;
				items[i] = item;
			}
	...
	TreeItem_old findItem(String value, TreeItem_old[] items) {
	...
	void packColumns (Tree_old tree) {
		if (multipleColumns.getSelection()) {
			int columnCount = tree.getColumnCount();
			for (int i = 0; i < columnCount; i++) {
				TreeColumn_old TreeColumn_old = tree.getColumn(i);
				TreeColumn_old.pack();
			}
		}
	}
	...
	void setColumnsMoveable () {
		...
		TreeColumn_old[] columns1 = tree1.getColumns();
		for (TreeColumn_old column : columns1) {
			column.setMoveable(selection);
		}
		TreeColumn_old[] columns2 = tree2.getColumns();
		for (TreeColumn_old column : columns2) {
			column.setMoveable(selection);
		}
	}
	 ...
	 * TreeItem_olds to default settings.
	 */
	@Override
	...
	/**
	 * Sets the background color of the Node 1 TreeItem_olds in column 1.
	 */
	...
	/**
	 * Sets the foreground color of the Node 1 TreeItem_olds in column 1.
	 */
	...
	/**
	 * Sets the font of the Node 1 TreeItem_olds in column 1.
	 */
	void setCellFont () {
		if (!instance.startup) {
	...
	/**
	 * Sets the background color of the Node 1 TreeItem_olds.
	 */
	...
	/**
	 * Sets the foreground color of the Node 1 TreeItem_olds.
	 */
	...
	/**
	 * Sets the font of the Node 1 TreeItem_olds.
	 */
	void setItemFont () {
		if (!instance.startup) {
	 ...
	 */
	void initializeSortState (final Tree_old tree) {
		/* Reset to known state: 'down' on column 0. */
		tree.setSortDirection (SWT.DOWN);
		TreeColumn_old [] columns = tree.getColumns();
		for (int i = 0; i < columns.length; i++) {
			...
			SelectionListener listener = widgetSelectedAdapter(e -> {
				...
				if (e.widget == tree.getSortColumn()) {
					...
					tree.setSortColumn((TreeColumn_old)e.widget);
				}
	...
	void resetSortState (final Tree_old tree) {
	...
	protected void specialPopupMenuItems(Menu menu, Event event) {
		...
		final Tree_old t = (Tree_old) event.widget;
